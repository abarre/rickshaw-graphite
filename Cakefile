...
build = (callback) ->
  launch 'node', ['vendor/build.js'], callback
...
mocha = (callback) ->
  launch 'open', ['SpecRunner.html'], callback
...
docco = (callback) ->
  walk 'src/js', (err, files) -> launch 'docco', files, callback
...
# cake build
# ```
task 'build', 'compile source', -> build -> log "Build successful", green
...
# cake test
# ```
task 'test', 'run tests', -> mocha -> log "Tests started", green
